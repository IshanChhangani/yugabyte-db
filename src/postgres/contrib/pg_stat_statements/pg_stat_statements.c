...
pgss_ExecutorEnd(QueryDesc *queryDesc)
{
	...
	if (queryId != UINT64CONST(0) && queryDesc->totaltime && pgss_enabled())
	{
		...
		pgss_store(queryDesc->sourceText,
				...
				//    (queryDesc->totaltime->total + yb_session_stats.exponential_backoff_time + yb_session_stats.retry_execution_time) * 1000.0,
				   (queryDesc->totaltime->total + YbGetBackoffTime() + YbGetRetryExecutionTime()) * 1000.0,
					...
					DestReceiver *dest, char *completionTag)
{
		...
		!IsA(parsetree, DeallocateStmt))
	{
		...
		pgss_store(queryString,
				   ...
				   pstmt->stmt_location,
				   pstmt->stmt_len,
				//    INSTR_TIME_GET_MILLISEC(duration) + ((yb_session_stats.exponential_backoff_time + yb_session_stats.retry_execution_time) * 1000.0),
				   INSTR_TIME_GET_MILLISEC(duration) + ((YbGetBackoffTime() + YbGetRetryExecutionTime()) * 1000.0),
				   rows,
				   &bufusage,
				   NULL);
	}
...
#define PG_STAT_STATEMENTS_COLS			24	/* maximum of above */
