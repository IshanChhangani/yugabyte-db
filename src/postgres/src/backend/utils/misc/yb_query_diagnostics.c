...
void YbQueryDiagnosticsInstallHooks(void)
{
    ...
    fprintf(fptr, "entered install hooks...\n");
    fclose(fptr);
...
bool
yb_enable_query_diagnostics_check_hook(bool *newval, void **extra, GucSource source)
{

    if (*newval && !yb_enable_query_diagnostics)
	{
		GUC_check_errdetail("query diagnostics must be enabled.");
		return false;
	}
	return true;
}
